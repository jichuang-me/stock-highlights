          ...
          <section className="space-y-6">
            ...
            <Card className="rounded-[30px] border-white/10 bg-white/[0.03] text-white shadow-none">
              <CardHeader className="flex flex-col gap-4 border-b border-white/10 pb-5 md:flex-row md:items-start md:justify-between">
                ...
                <button
                  className="flex items-center gap-3 rounded-2xl border border-white/10 bg-slate-900/80 px-4 py-3 text-left transition hover:border-white/20"
                  onClick={() => setModelDialogOpen(true)}
                  type="button"
                >
                  <Settings2 className="h-4 w-4 text-cyan-300" />
                  <div>
                    <div className="text-sm font-medium text-white">{activeProfile.label}</div>
                    <div className="text-xs text-slate-400">
                      {profileKindLabel[activeProfile.kind]} · {activeProfile.mode === 'server' ? '服务端' : '自定义端点'}
                    </div>
                  </div>
                </button>
              </CardHeader>
              ...
              <CardContent className="space-y-3">
                <div className="rounded-2xl border border-white/10 bg-slate-950/80 p-4">
                  <div className="flex items-center justify-between gap-3">
                    <span className="text-sm text-slate-400">模型定位</span>
                    <Badge className="border-white/10 bg-white/5 text-slate-200">
                      {profileKindLabel[activeProfile.kind]}
                    </Badge>
                  </div>
                  <div className="mt-2 break-all text-sm text-white">
                    {activeProfile.model || '按服务器优先级自动选择'}
                  </div>
                </div>

                <Button className="w-full rounded-2xl" onClick={() => setModelDialogOpen(true)} type="button">
